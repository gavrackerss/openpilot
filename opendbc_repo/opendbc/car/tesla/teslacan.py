...
class TeslaCAN:
  ...
  @staticmethod
  def checksum(msg_id: int, dat) -> int:
    ret = (int(msg_id) & 0xFF) + ((int(msg_id) >> 8) & 0xFF)
    ret += sum(dat)
    return ret & 0xFF

  def create_legacy_hud_status(self, enabled: bool, speed_limit_uom: float, bus: int, counter: int,
                               hands_on_state: int = 0, alca_state: int = 1,
                               blind_spot_left: bool = False, blind_spot_right: bool = False):
    speed_limit = max(0.0, min(float(speed_limit_uom), 150.0))
    values = {
      "autopilotStatus": 5 if enabled else 2,
      "DAS_blindSpotRearLeft": 1 if blind_spot_left else 0,
      "DAS_blindSpotRearRight": 1 if blind_spot_right else 0,
      ...
      "DAS_statusCounter": int(counter) % 16,
      "DAS_statusChecksum": 0,
    }
    data = self.packer.make_can_msg("AutopilotStatus", int(bus), values)[1]
    values["DAS_statusChecksum"] = self.checksum(0x399, data[:7])
    return self.packer.make_can_msg("AutopilotStatus", int(bus), values)

  def create_legacy_hud_status2(self, acc_speed_limit_uom: float, bus: int, counter: int):
    speed_limit = max(0.0, min(float(acc_speed_limit_uom), 204.6))
    values = {
      "DAS_accSpeedLimit": speed_limit,
      ...
      "DAS_longCollisionWarning": 15,
      "DAS_status2Counter": int(counter) % 16,
      "DAS_status2Checksum": 0,
    }
    data = self.packer.make_can_msg("DAS_status2", int(bus), values)[1]
    values["DAS_status2Checksum"] = self.checksum(0x389, data[:7])
    return self.packer.make_can_msg("DAS_status2", int(bus), values)

  @staticmethod
  def create_legacy_hud_warning_matrix0(bus: int):
    return (0x329, bytes(8), int(bus))

  @staticmethod
  def create_legacy_hud_warning_matrix1(bus: int):
    return (0x369, bytes(8), int(bus))

  @staticmethod
  def create_legacy_hud_warning_matrix3(bus: int):
    return (0x349, bytes(8), int(bus))
