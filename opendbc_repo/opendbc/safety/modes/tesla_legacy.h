...
static const char *const xnor_v198_marker __attribute__((unused)) =
...
// sub-18. No checksum/counter on this frame, so only byte7 changes. Compile-time toggle:
//   1 = override enabled (the experiment), 0 = leave GTW_carConfig untouched (safe default).
// CAVEATS (why this is expected to be LOW-probability): (a) same topology wall as 0x2B9 — only
// affects the forwarded copy, not a native segment the panda isn't between; (b) ECUs usually
...
// forwarded GTW_carConfig frame. No checksum/counter on 0x398, so ONLY byte7 changes. Verified
// against real payloads: user byte7=0x00 (autopilot=0) -> 0x20 (autopilot=2); all other bytes
// identical. Returns true if it changed the frame.
static bool __attribute__((unused)) tesla_legacy_override_gtw_autopilot(CANPacket_t *msg) {
  const uint8_t want = (uint8_t)((msg->data[7] & 0xCFU) | 0x20U);  // bits 4-5 = 0b10 (=2)
  if (msg->data[7] == want) {
    return false;   // already 2, nothing to do
  }
  msg->data[7] = want;
  return true;
}
...
static bool tesla_legacy_is_hud_status_msg(int addr) {
  // 0x399/0x389 are status overlays; 0x239 is the fused-lane overlay; 0x488 is the
  // Hybrid Native AP steering template. 0x2B9 is the
  // Unity-style chassis DAS_control overlay used for stop/go: userspace authors the desired
  // payload, but panda never transmits that packet directly. Instead it is merged onto the
  // genuine AP 0x2B9 so the AP cadence/counter remain authoritative.
  return (addr == 0x399) || (addr == 0x389) || (addr == 0x239) || (addr == 0x2B9) || (addr == 0x488);
}
...
static TeslaLegacyHudForwardData tesla_legacy_hud_forward_data[] = {
  ...
  // Hybrid DAS_steeringControl: preserve genuine AP 4-bit counter in byte2 low nibble;
  // replace OP angle/type/haptic and recompute the additive checksum in byte3.
  {0x488, 2, 0, 4, 100000U, 0x00000000U, 0x000F0000U, 0U, 0U, 0U, 0U, false},
  // DAS_control chassis: preserve genuine AP counter (bits 53..55); recompute additive checksum.
  {0x2B9, 2, 0, 8, 50000U, 0x00E00000U, 0x00000000U, 0U, 0U, 0U, 0U, false},
};
...
static bool tesla_legacy_hud_tx_has_valid_counter(const CANPacket_t *msg, const TeslaLegacyHudForwardData *fwd) {
  ...
  const uint32_t data_h = (len > 4) ? tesla_legacy_get_u32_le(&msg->data[4]) : 0U;
...
static bool tesla_legacy_apply_hud_forward_data(CANPacket_t *to_fwd, int bus_num) {
  ...
  for (int i = 0; i < fwd_len; i++) {
    ...
    if ((addr == fwd->addr) && (bus_num == fwd->source_bus) && (len == fwd->len)) {
      ...
      // V199 co-op re-arm phase. The V198 log showed EPAS stuck EAC_AVAILABLE/HANDS_ON while
      // active type-1 overlays continued, so it never saw a neutral interval in which to reset.
      // Preserve Tesla's genuine cadence, angle, counter and all other bits, but force controlType
      // 0 until real EPAS feedback is healthy. This applies even without a fresh OP template.
      if ((addr == 0x488) && tesla_legacy_coop_rearm_active) {
      ...
      // DAS_status/DAS_status2/DAS_control and DAS_steeringControl use Tesla's additive
      // last-byte checksum. DAS_lanes uses byte7 high nibble as counter and no checksum.
      if ((addr == 0x399) || (addr == 0x389) || (addr == 0x2B9) || (addr == 0x488)) {
        tesla_legacy_set_last_byte_checksum(to_fwd);
      }
...
static void tesla_legacy_rx_hook(const CANPacket_t *msg) {
  ...
  if (bus == tesla_legacy_chassis_bus) {
    if (addr == id_pedal) {  // DI_torque1
      ...
      // STANDBY/OVERRIDE/ENABLED transitions must never erase or create OP controls_allowed.
  ...
  if (bus == 2) {
    ...
    if (!tesla_legacy_external_panda && tesla_legacy_has_ap_hw) {
      if (addr == 0x399) {
        ...
        // Autopark always wins. Ordinary stock AP/EAC also wins in native mode, but in explicit
        // Autopilot-Disabled mode those AP status bits are informational only because panda blocks
        // the stock steering carrier before it reaches EPAS.
        if (tesla_legacy_autopark_enabled ||
            (!tesla_legacy_op_hybrid_native_ap && !tesla_legacy_op_autopilot_disabled &&
             (tesla_legacy_autopilot_enabled || tesla_legacy_eac_enabled))) {
          controls_allowed = false;
        }
      } else {
      }
...
// here that a direct DAS_control command would receive.
static bool tesla_legacy_chassis_overlay_violation(const CANPacket_t *msg) {
...
static bool tesla_legacy_hybrid_steering_overlay_violation(const CANPacket_t *msg) {
  ...
  const int steer_control_type = (int)((msg->data[2] >> 6) & 0x03U);
  if (steer_control_type != 1) {  // hybrid accepts only active OP ANGLE_CONTROL templates
    return true;
  }
...
static bool tesla_legacy_tx_hook(const CANPacket_t *msg) {
  ...
  // NEVER transmit that userspace frame directly. It is applied only to a genuine AP 0x2B9 in
  // the bus2->bus0 forward hook, preserving native timing/counter and avoiding the EPAS inhibit
  // caused by an unsolicited second 0x2B9 sender.
  if (!tesla_legacy_external_panda && (addr == 0x2B9) && ((int)msg->bus == 0)) {
    const bool violation = tesla_legacy_chassis_overlay_violation(msg);
  ...
  // 0x488 is excluded here because outside Hybrid mode it follows the existing direct steering path.
  if ((addr != 0x488) && tesla_legacy_capture_hud_tx(msg, false)) {
    return false;
  }

  // Hybrid EPAS recovery. Permit only APS_eacAllow=1 on the main panda/bus0 while OP remains
  // authorised. V186 covers native-LKAS active->idle; V199 starts the same allow handshake at
  // physical co-op takeover while panda holds the car-facing steering carrier neutral.
  ...
  // tightly-gated V186/V199 recovery cases handled immediately above. Hybrid exempts only
  // DAS_control here for the direct OP 0x2BF path; main-panda 0x2B9 remains native-only.
  if (tesla_legacy_has_ap_hw && !tesla_legacy_op_autopilot_disabled) {
    if ((addr == 0x488) || (addr == 0x27D) ||
        (tesla_legacy_is_das_control_msg(addr) && !tesla_legacy_op_hybrid_native_ap)) {
      return false;
    }
  }
  ...
  if (!tesla_legacy_external_panda) {
    ...
    if (addr == 0x488) {
      ...
      // In ordinary/native mode never compete with genuine stock LKAS. In explicit
      // Autopilot-Disabled mode the forward hook below suppresses the stock 0x488/0x27D stream,
      // so the raw AP-side stock_lkas bit must not block OP's single direct steering source.
      if (tesla_legacy_stock_lkas && !tesla_legacy_op_autopilot_disabled) {
        return false;
      }
...
static bool tesla_legacy_fwd_msg_hook(int bus_num, CANPacket_t *to_fwd) {
  ...
  // forwarding path, UNCONDITIONALLY (feature coding is broadcast regardless of engagement — the
  // factory car advertises autopilot=2 continuously, so we mirror that). No engage/speed gate.
  if (addr == 0x398) {
    (void)tesla_legacy_override_gtw_autopilot(to_fwd);
  }
#endif
  ...
  // Experimental native-Autosteer presentation-state test. Dashcam captures show an otherwise
  // undecoded 3-bit field in 0x247 byte5[7:5] switching 2 -> 5 whenever genuine Tesla Autosteer
  // becomes active (and 5 -> 2 on disengage). In normal xnor Autopilot-Disabled mode only, force
  // that field to 5 while OP controls are engaged. Hybrid mode never touches it: native AP owns it.
  // If this frame uses Tesla's common additive last-byte checksum, detect that from the incoming
  // stock frame and recompute it; otherwise preserve byte7 unchanged.
  if ((bus_num == 2) && (addr == 0x247) && tesla_legacy_autosteer_247_test &&
      tesla_legacy_op_autopilot_disabled && !tesla_legacy_op_hybrid_native_ap && controls_allowed) {
    const int len = GET_LEN(to_fwd);
    const bool additive_checksum = (len == 8) && (to_fwd->data[7] == tesla_legacy_calc_checksum8(to_fwd, len));
...
static safety_config tesla_legacy_init(uint16_t param) {
  ...
  static const CanMsg TESLA_LEGACY_TX_MSGS_LATERAL[] = {
    ...
    {0x2B9, 0, 8, .check_relay = false},  // DAS_control TEMPLATE only; validated/captured, never direct TX
