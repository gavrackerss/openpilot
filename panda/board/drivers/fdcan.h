...
void can_rx(uint8_t can_number) {
  ...
  while((FDCANx->RXF0S & FDCAN_RXF0S_F0FL) != 0U) {
    ...
    // New opendbc API: safety_fwd_hook takes a mutable CANPacket_t* so the mode can
    // both route and (if it wants) modify the frame. Pass a cleared copy of to_push;
    // tesla_legacy forwards the HUD frames (0x399/0x389) unchanged, so this stays
    // vanilla-style and the IC sees one continuous warning stream.
